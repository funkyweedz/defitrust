...
const baseProjects = [
  ...
  },
  {
    ...
    verified: true,
    score: 85,
    scoreBreakdown: "Base 100 · Notion share link expired (broken) −15 · host on GitHub or IPFS to restore score",
    avatar: "rose",
    website: "https://ellipsis.trade",
    hash: "ellipsis_phoenix_audit_final.pdf — OtterSec",
    summary:
      "Trust score 85/100 — 1 audit by OtterSec covering the Phoenix on-chain CLOB. No critical issues found. The original Notion share link expired on May 12, 2026. Score: base 100 − 1 broken link (−15). Uploading to GitHub or IPFS would restore the score to 100.",
    audits: [
      {
        auditor: "OtterSec",
        date: "2024",
        severity: "Phoenix CLOB security audit · Critical: 0 — link expired, re-host needed",
        scope: "Phoenix on-chain central limit order book (Solana)",
        certificate: "ellipsis_phoenix_audit_final.pdf",
        reportUrl: "https://github.com/Ellipsis-Labs/phoenix-v1",
        flags: ["broken"],
      },
    ],
  },
  ...
  projectGrid.innerHTML = filtered
    .map(
      (project) => {
        ...
        const accent = project.brandColor || '#f39322';
        const accentGlow = hexToRgba(accent, 0.13);
        const accentFaint = hexToRgba(accent, 0.07);
        return `
        <article class="project-card" data-verified="${project.verified}" style="--accent:${accent};--accent-glow:${accentGlow};--accent-faint:${accentFaint}">
          ...
          <div class="card-stats">
            <div class="card-stat">
              ...
              <span>${project.reports > 1 ? "reports" : "report"}</span>
            </div>
            <div class="card-stat-divider"></div>
            <div class="card-stat">
              <strong>${escapeHtml(project.tvl)}</strong>
              <span>TVL</span>
            </div>
          </div>
